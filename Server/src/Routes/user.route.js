...
import userUpload from '../Middlewares/multer.config/user.multer.middleware.js'
...
router.route('/:id/reset-password').put(verifyJWT, validateObjectId('id'), validate(resetPasswordValidation, validationSource.BODY), resetCurrentPassword)
router.route('/:id/update').put(verifyJWT, userUpload.single("avatar"), validate(updateUserValidation, validationSource.BODY), updateUser)
